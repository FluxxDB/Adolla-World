...
function transformExpression(state, expression) {
    return diagnosticsUtils_1.catchDiagnostic(expression, function () {
        var transformer = TRANSFORMERS.get(expression.kind);
        if (transformer) {
            return transformer(state, expression);
        }
        return typescript_1.default.visitEachChild(expression, function (newNode) { return transformNode_1.transformNode(state, newNode); }, state.context);
    });
}
