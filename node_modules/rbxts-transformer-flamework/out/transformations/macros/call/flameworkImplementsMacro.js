...
exports.FlameworkImplementsMacro = {
    ...
    transform: function (state, node) {
        ...
        if (!symbol)
            diagnostics_1.Diagnostics.error(firstType, "Could not find symbol for type");
        var declaration = (_b = symbol.declarations) === null || _b === void 0 ? void 0 : _b[0];
        if (!declaration)
            diagnostics_1.Diagnostics.error(firstType, "Could not find declaration for type");
        var importId = state.addFileImport(state.getSourceFile(node), "@rbxts/flamework", "Flamework");
        return factory_1.f.call(factory_1.f.field(importId, "_implements"), [node.arguments[0], state.getUid(declaration)], node.typeArguments ? __spreadArray([], __read(node.typeArguments)) : []);
    },
};
